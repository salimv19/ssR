...
R tiene 6 clases atómicas^[Los tipos básicos son referidos como atómicos cuando es necesario excluir listas.] [@rmanual].
...
```{r, echo=FALSE}
dfTab <- data.frame(
  a <- c("`character`", "`numeric`", "`integer`", "`logical`", "`complex`", "`raw`"),
  b <- c("Caracter", "Numérico", "Integer", "Lógico", "Complejo", "Crudo"),
  c <- c("'hola', 'x'", "67, 45.5", "2L, 67L", "TRUE, FALSE, T, F", "1+4i", "01 - imprime hexadecimales")
)
colnames(dfTab) <- c("Type","Tipo", "Ejemplo")
...
Los vectores son la estructura de datos más básica de R [@wickham2014advanced]. Hay dos tipos 
...
datos [@rmanual], es decir, elementos de alguna de las clases atómicas 
...
coerción puede derivar en pérdida de información [@wickham2014advanced].
...
```{block, type='nota'}
Lo importante es recordar que es importante revisar las **advertencias**
que `R` arroja a la consola y verificar que el resultado obtenido es el 
...
de un vector a través de operaciones de indexación (*indexing*) [@rmanual, sección "Indexing"].
...
En vectores, `[[` casi no se utiliza, aunque son ligeramente diferentes. Como 
vimos en el ejemplo, `[[` quita los nombres o atributos y permite extraer
...
En matrices, `[[` casi no se utiliza. Como 
vimos en el ejemplo, `[[` quita los nombres o atributos y permite extraer
únicamente un elemento a la vez.
```
...
también se les conoce como vectores recursivos (*recursive vectors*) [@wickham2014advanced, sección "lists"].
...
En listas, `[[` es fundamental para accesar correctamente los objetos y 
poder navegar la lista. 

Como en vectores y matrices, `[[` quita los nombres o atributos y permite extraer únicamente un elemento a la vez. En listas, además, devuelve el objeto dentro 
del i-ésimo elemento. Por el contrario, `[` devuelve una lista.
...
`R` funciona mejor gracias a sus convenciones, es decir, porque
...
La función `summary` es la función genérica que produce resumenes para objetos
...
likert ^[Se denomina así por su autor quien en [@likert1932] describe su uso.]
...
datos en R [@wickham2014advanced, sección "Data frames"]. En `python`, existe
...
específico como `R` [@pandas, sección "What problem does pandas solve?"].
...
en la sección [5.1](#importacion-de-datos).
...
Por *default* las columnas tipo caracter en un `dataframe` son
...
recomendable cambiar la opción por *default* para leer columnas
...
Debido a que los *dataframes* son un caso particular de las listas,
`[[` es también fundamental para accesar correctamente los objetos. 

`[[` quita los nombres o atributos, permite extraer
únicamente un elemento a la vez y devuelve el objeto dentro 
del i-ésimo elemento. 

`[` devuelve un data frame con la columna(s) que sea nombradas o los índices
que sean utilizados.
```
...
Los `dataframes` pueden tener únicamente nombres sintácticamente 
válidos [@rfaq, sección "What are valid names?"]:
...
Fuera del @rbase, se han desarrollado otras estructuras de datos particularmente
...
@rbase. Está implementado en el paquete del mismo nombre [@datatable].
...
a los *defaults* para facilitar el trabajo [@wickham2016r, ver sección "tibbles"]. 

Un `tibble` se crea en forma análoga a un `data.frame`. En la sección [](#),
creamos un objeto llamado $df$
...
Usamos la función `tibble()` del paquete @tibble:
...
válidos, para utilizar estos nombres, se declaran con acento invertido ```
...
pueden incorporar elementos de listas en una columna en forma sencilla [@wickhammx].
...
En la sección [3.5](#objetos-importantes) se habló de otros objetos en R.
