...
knitr::opts_chunk$set(echo = T, error = F, message = F, warning = F, results = 'hold')
```
...
\begin{figure}[h]
    ...
    \caption{Transformación de datos [@grolemund2016r, Introducción].}
    \label{fig:ciclo3}
\end{figure}
...
\begin{enumerate}
\item Revisa la ayuda de select con `?select`.
\item Juega con la función `starts\_with()`. ¿Qué variables empiezan con ``de''?
\item Juega con la función `ends\_with()`. ¿Qué variables terminan con ``delay''?
\item Utiliza la base de datos `iris` como en el ejemplo de la ayuda.
\item ¿Qué hace la función `contains()`?
\item ¿Cómo es diferente de `matches()`?
\item ¿Cómo obtienes todas las variables *menos* Petal.Width?
\end{enumerate}
\end{bclogo}
...
\begin{itemize}
\item `first`: extrae el primer valor de un vector
\item `last`: extrae el último valor de un vector
\item `n`: cuenta el número de valores en un vector
\item `n\_distinct` cuenta el número de valores único en un vector
\end{itemize}
\end{curiosidad}
...
\item Utiliza la función `str\_sub` dentro de `stringr` para extraer únicamente el 
día del campo `date`.
3. Utiliza la función `ymd` del paquete `lubridate` para declarar date como una
fecha (¡otra clase!).
4. Utiliza las funciones `paste0` del `base` y `ymd\_hm` de `lubridate` para
...
\begin{curiosidad}[Funciones ventana *window functions*]
...
\begin{itemize}
\item `lead`: regresa todos los valores del vector movidos por una posición posterior
\item `lag`: regresa todos los valores del vector movidos por una posición anterior
\item `dense\_rank`: rango sin huecos
\item `min\_rank`: rango especificando el criterio de mínimo para empates
\item `percent\_rank`: rango reescalado para estar entre 0 y 1
\item `row\_number`: número de fila
\item `ntile`: creación de $n$ percentiles
\item `between`: verifica si el valor está entre dos valores
\item `cume\_dist`: distribución acumulada
\item `cumall`: para vectores lógicos, intersección de los valores al renglón i-ésimo
\item `cumany`: para vectores lógicos, unión de los valores al renglón i-ésimo
\item `cummean`: acumula la media 
\end{itemize}
...
\begin{figure}[h]
    ...
    \caption{Joins en el lenguaje SQL [@joins].}
    \label{fig:joins}
\end{figure}
...
\begin{bclogo}[
  ...
  noborder=true]{Ejercicios}
\begin{enumerate}
\item Busca en la ayuda la función `top\_n`.
\item Utilízala, junto con `group\_by`, `arrange` y `summarise` para extraer los 10 
...
\begin{curiosidad}[Afectación de otros verbos por `group\_by`]
...
\begin{itemize}
\item `select` realiza la misma operación pero retiene siempre las variables de agrupación.
\item `arrange` realiza el arreglo según las variables especificadas pero ordena primero según los grupos.
\item `mutate` y `filter` realizan las operación dentro de los grupos definidos y son más útiles cuando se utilizan en conjunción a las funciones ventana.
\item `sample\_n` y `sample\_frac` extraen el número o fracción de observaciones según el número o fracción de filas en cada grupo.
\item `summarise` en conjunción con `group\_by` llevan acabo el paradigma split (por las variables definidas en el `group\_by`), apply (por las funciones especificadas en el `summarise`) y lo combinan en un dataframe.
\end{itemize}
\end{curiosidad}
...
\item Realiza un resumen de los vuelos por aeropuerto para varias variables con la función `summarise`.
\item Utiliza la función `inner\_join` para pegar la tablas de resumen creada en el paso anterior a la tabla *airports*
\item Realiza un resumen de los vuelos por avión para varias variables con la función `summarise`.
\item Utiliza la función `left\_join` para pegar la tablas de resumen creada en el paso anterior a la tabla *planes*
