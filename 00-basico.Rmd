...
para instalar R compilándolo [@instalarcompilado].
...
desarrollar [@rstudio]. En su versión más reciente, también autocompleta código 
...
los usuarios de `emacs` y `R` [@rossini2004ess]. Soporta la edición de scripts para R, S-plus, SAS,
...
posible continuar *desde donde te quedaste* [@kabacoff2015r, pág. 11].
...
2. Utiliza la tecla `tab` <img src="images/tab_key.jpg" width="32"> para autocompletar las posibles rutas desde donde quiera que estés.
...
El resultado que se muestra aquí al usar el comando `getwd()` depende de la computadora en la que se esta 
trabajando debido a que es una *ruta absoluta*. Nota como es diferente la ruta 
que obtienes al correr el comando en tu consola de `R`. Eso es porque se trata 
de una ruta absoluta, es decir, es tal que da la ruta (*path*) completo
...
de archivos o computadora en particular [@gandrud2013, pág. 67]. 
...
Estas operaciones también pueden ser realizadas entre vectores^[Revisaremos más adelante con detalle la definición de vectores en la sección [3](#estructuras-y-funciones).]. 
...
instalación estándar resumida en la sección [B-2](#instalacion-1),
...
*Comprehensive R Archive Network* [@cran] es una colección de sitios que contienen
...
@pacman pues provee de herramientas para instalar paquetes en una forma
...
Si queremos instalar varios paquetes usando las herramientas del R básico (*base*) [@rbase]
haríamos algo como [ejemplo tomado de @pacman, en la viñeta de introducción al paquete]:
...
```{block, type='curiosidad'}
Nota como se puede llamar a una función por su nombre `p\_load` una vez que 
ya cargamos el paquete en el cuál esa función está guardada con el comando `library(pacman)`
o podemos llamarla directamente utilizando la convención `paquete::funcion`, en este caso,
`pacman::p\_load`.
```
...
esto es utilizando `packrat`. Para mayor detalle, ver el apéndice [B](#packrat).
```
...
utilizamos la extensión **.R** o **.r**. En RStudio [@rstudio], `CTRL + SHIFT + N` abre 
...
De la misma manera, si utilizas `Emacs + ESS` [@rossini2004ess], existen múltiples atajos de teclado
...
alta calidad en múltiples formatos [@rmarkdownconcept]. Para utilizarlo,
...
[la documentación](http://rmarkdown.rstudio.com/html_document_format.html) [@rmarkdownhtml].
...
[la documentación](http://rmarkdown.rstudio.com/pdf_document_format.html) [@rmarkdownpdf]. 
Esta opción es conveniente cuando se tiene conocimiento previo de LaTeX.

Por último, cabe mencionar la opción `word_document`, cuyas opciones se encuentran [aquí](http://rmarkdown.rstudio.com/word_document_format.html) [@rmarkdownword] y la opción `md_document` que compila a Markdown.

Así como es posible generar documentos, es posible crear presentaciones en HTML (ioslides o slidy)
o pdf (beamer) [@rmarkdownreference] y dashboards (con flexdashboards) [@flexdashboard].

En el apéndice [A](#markdown) se detalla la sintaxis de Markdown, misma
...
con tres acentos invertidos [@rmarkdownreference].
...
código no se imprima y que solo se imprima el resultado agregando `{r, echo = FALSE}`:

```{r, echo=FALSE}
paste("Hola", "Mundo")
```

Las opciones se encuentran resumidas en @rmarkdownreference.
...
^[Hay otro tipo de funciones en las que accesar al código fuente no se pueda con los métodos descritos. Para ello, es útil revisar la sección "old-school object-oriented programming in R" [@adler2010r, págs. 131-133] o las secciones dedicadas a los objetos S3 y S4 en @wickham2014advanced.].
...
referirse a la comparación de rendimiento en @eddelbuettel2010 o
al paquete del mismo autor @gcbd.

Para la parte práctica de todo esto, referirse a 
[este blog](http://brettklamer.com/diversions/statistical/faster-blas-in-r/)
para instalar las librerías apropiadas para BLAS y Lapack [@blasinr]. Para una comparación
bastante práctica de las diferentes versiones de esas librerías, ver
[aquí](http://blog.nguyenvq.com/blog/2014/11/10/optimized-r-and-python-standard-blas-vs-atlas-vs-openblas-vs-mkl/) [@optimizedr].
...
aprender `R` es `swirl` [@swirl], un paquete en CRAN en el que se 
...
<img src="images/swirl_commands.png" width="800" alt="Swirl_commands" align="middle">
...
<img src="images/swirl_install_course.png" width="800" alt="Swirl_install_course" align="middle">
