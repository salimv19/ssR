...
de datos limpios presentada en @wickham2014tidy e implementada en el paquete `tidyr`
[@tidyr]. En la figura [5.5](#fig:ciclo2) podemos ver la etapa del
análisis de datos correspondiente.

```{r ciclo2, out.width='75%', fig.show='hold', fig.align = 'center', fig.cap='Limpieza de datos [@grolemund2016r, Introducción].', echo = FALSE, results='hold'}
knitr::include_graphics('images/02_ciclo_2.png')
```
...
utilizado, son particularmente útiles en `R` pues éste es un lenguaje
...
Tomamos el ejemplo de datos de la figura [5.6](#fig:estructura) en donde se presentan 
datos de un experimento^[Ejemplo tomado de [@wickham2014tidy, pág. 3].]. La tabla contiene dos columnas y tres filas, 
ambas etiquetadas.

```{r estructura, out.width='40%', fig.show='hold', fig.align = 'center', fig.cap='Típica presentación de datos.', echo = FALSE, results='hold'}
knitr::include_graphics('images/02_estructura.png')
```
...
se muestra en la figura [5.7](#fig:estructurat). El diseño cambia pero los datos
son los mismos. Además de la apariencia, deberíamos de poder describir la 
semántica -el significado- 
de los valores que se muestran en una tabla [@wickham2014tidy, pág. 3]
pero la abstracción de filas y columnas no da para más.

```{r estructurat, out.width='50%', fig.show='hold', fig.align = 'center', fig.cap='Mismos datos que en [5.6](#fig:estructura) pero traspuestos.', echo = FALSE, results='hold'}
knitr::include_graphics('images/02_estructurat.png')
```
...
Los mismos datos en las figuras [5.6](#fig:estructura) y [5.7](#fig:estructurat) los
...
En la figura 
[5.8](#fig:estructuratidy) se muestran los mismos datos que antes pero pensados tal que las 
variables son columnas y las observaciones (en este caso, cada punto en el
diseño experimental) son filas.

```{r estructuratidy, out.width='40%', fig.show='hold', fig.align = 'center', fig.cap='Observaciones son filas, variables columnas.', echo = FALSE, results='hold'}
knitr::include_graphics('images/02_estructuratidy.png')
```
...
promedio, la varianza, la moda [@wickham2014tidy, pág. 4]. 
...
En la figura [5.9](#fig:datoslimpios) podemos ver estos tres elementos de los 
datos limpios y cómo se representan en un *dataframe*.

```{r datoslimpios, out.width='90%', fig.show='hold', fig.align = 'center', fig.cap='Ejemplificación de datos limpios [@wickham2016r, sección Data Tidying].', echo = FALSE, results='hold'}
knitr::include_graphics('images/datos_limpios.png')
```

Esto equivale a la tercera forma normal de Codd [@wickham2014tidy, pág. 4] 
...
La tabla [5.8](#fig:estructuratidy) corresponde a datos limpios: cada fila es
...
en Tabla [5.6](#fig:estructura).

```{block, type='ejercicio'}
**Ejercicios**

1. Crea un dataframe con los valores de la tabla [5.6](#fig:estructura) y otro 
con los valores de la tabla [5.8](#fig:estructuratidy).
...
Los 5 problemas más comunes para llevar datos sucios a limpios [@wickham2014tidy, pág. 5] son:
...
en el paquete `tidyr`: `gather`, `spread`, `separate` y `unite`. 

#### Los nombres de las columnas son valores, no nombres de variables

La tabla [5.2](#tab:varsencols) muestra datos sucios con este problema. La base
...
sentido. Por ejemplo, en la tabla [5.2](#tab:varsencols) vemos los casos 
de tuberculosis para distintos grupos de edad de hombres en México para cierto
tipo de diagnóstico.

```{r varsencols, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, results='hold'}
raw <- filter(who, country == "Mexico", year >= 2000, year < 2011) %>% 
  select(., country, year, contains("sp_m"))

knitr::kable(
  raw, booktabs = TRUE,
  caption = 'Casos de tuberculosis para México del 2000 al 2010 <br>para hombres con diagnóstico por lesiones de pulmón.'
) %>%
  kable_styling() %>%
...
```{r varsjuntadas, echo = FALSE, results = "asis"}
 knitr::kable(head(junta), booktabs = TRUE,
...
16 sujetos a lo largo de un año en donde se mide el colesterol una vez al mes (mes1, mes2, etc.). Los datos capturados se muestran en la tabla [5.4](#tab:sujetos). 
...
muestra en la tabla [5.5](#tab:sujetostidy).
```


```{r sujetos, echo=FALSE, results='asis'}
...
```{r sujetostidy, echo=FALSE, results='asis'}
# Respuesta
...
En esencia, las *regex* (por el inglés regular expressions) utilizan 
...
[@peng2016m, sección "text processing and regular expressions"] 
realiza una buena revisión de las funciones disponibles en `R` para 
...
Nota como en el último ejemplo, se utiliza el símbolo `%<>%` que es 
equivalente a realizar una asignación (<-) y un pipe (`%>%`) de los datos 
que están guardados en el dataframe limpios.
```

De esta forma, obtenemos los datos como se ven en la tabla [5.6](#tab:varslimpios)
...
```{r varslimpios, echo = FALSE, results = "asis"}
knitr::kable(head(limpios), booktabs = TRUE,
...
y mínima en algunas zonas de México [@tidydata, archivo: data/weather.txt].
Los datos que limpiaremos se ven en la tabla [5.7](#tab:clima). Como se puede 
...
```{r clima, echo=FALSE, results='asis'}
rm(list = ls())
source("weather.R")

knitr::kable(raw[, 1:15], booktabs = TRUE,
...
para crear la fecha. Así, obtenemos la tabla [5.8](#tab:clima1), a partir de 
...
La [documentación](https://cran.r-project.org/web/packages/stringr/stringr.pdf)
detalla todas sus funciones. En este caso, utilizamos la función `str_replace`
que nos permite reemplazar una cadena de caracteres por otra.
```

```{r clima1, echo=FALSE, results='asis'}
knitr::kable(clean1[1:5, ], booktabs = TRUE,
caption = 'Paso 1. Juntar las columnas, limpiar días, crear fecha.'
)
```
...
```{r clima2, echo=FALSE, results='asis'}
knitr::kable(clean1[1:5, ], booktabs = TRUE,
...
Para este ejemplo, utilizamos la base de datos `billboard` [@tidydata, archivo: data/billboard.csv]
...
Este ejemplo y datos se toman de @tutorialintro.
...
`M2_219.xlsx` y se puede observar un ejemplo de éstos en la figura [5.10](#fig:ejerciciofinal).
El ejercicio consiste en leer los datos y limpiarlos de forma que sean datos
limpios.

*Los datos fueron tomados de [@informe, págs. 218-219] y el catálogo de estados se tomó de [@mgn].*

```


```{r ejerciciofinal, out.width='75%', fig.show='hold', fig.align = 'center', fig.cap='Datos de pensiones del IMSS e ISSSTE 2000 a 2016 [@informe, págs. 218-219].', echo = FALSE, results='hold'}
knitr::include_graphics('images/ejercicio_tidy_informe.png')
```
